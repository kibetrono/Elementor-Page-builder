...
define( 'W8CONTACT_FORM_VERSION' , '1.5.8' );
	...
	class contact_form_slider {
		...
		$params = $contact_form_slider->get_parameters();
			...
			if ( isset( $custom_fields[ 0 ] ) ) {
				foreach( $custom_fields[ 0 ] as $key => $cf ) {
					if ( $cf->id == 'TRPHONE' ) {
						$custom_fields[ 0 ][ $key ]->name = esc_html__( 'Phone Number', W8CONTACT_FORM_TEXT_DOMAIN );
					}
				}
			}
		/* params for demo end */
		...
		**/
		...
		public function get_parameters()
		{
			...
			if (get_option('setting_default_translation')=="on")
			{
				$params['placeholder_name'] = esc_html__( 'Enter your name', W8CONTACT_FORM_TEXT_DOMAIN );
				$params['placeholder_email'] = esc_html__( 'Enter your email address', W8CONTACT_FORM_TEXT_DOMAIN );
				$params['placeholder_message'] = esc_html__( 'Type your message...', W8CONTACT_FORM_TEXT_DOMAIN );
				$params['placeholder_captcha'] = esc_html__( 'Enter the numbers', W8CONTACT_FORM_TEXT_DOMAIN );
				$params['placeholder_sendcopy'] = esc_html__( 'Send a copy to my email address', W8CONTACT_FORM_TEXT_DOMAIN );
				$params['sendbutton_text'] = esc_html__( 'Send', W8CONTACT_FORM_TEXT_DOMAIN );
				$params['success_message'] = esc_html__( 'Message sent successfully', W8CONTACT_FORM_TEXT_DOMAIN );
				$params['failed_text'] = esc_html__( 'FAILED', W8CONTACT_FORM_TEXT_DOMAIN );
			}
			$blogemail = get_bloginfo( 'admin_email' );
			$be = explode('@',$blogemail);
		...
		public function ajax_cfs() {
				...
				if (!empty($cfs_sql))
				{
					foreach($cfs_sql as $cs)
					{
						...
						if (isset($datas['data']['autoreply']))
						{
							...
							$clientcopy = "<i>".esc_html__( 'Copy of the email has not been sent to the Client', W8CONTACT_FORM_TEXT_DOMAIN )."</i>";
							if (isset($datas['data']['mail']['copy']))
							{ 
								if ($datas['data']['mail']['copy']=="true") $clientcopy = "<i>".esc_html__( 'Copy of the email has been sent to the Client', W8CONTACT_FORM_TEXT_DOMAIN )."</i>";
							}
			...
			if ( isset( $_REQUEST[ 'sendc' ] ) ) {
				...
				$message = esc_html__( 'Sender: ') . ' ' . $name.'  &lt;' . $semail . '&gt;<br /><br />' . nl2br( $message ) . '
